...
export async function flashFirmware(config: FlashConfig): Promise<string> {
  ...
  if (config.eraseAll) args.push("erase_flash");
  ...
  return new Promise((resolve, reject) => {
    ...
    esptool.on("close", (code) => {
      ...
      else reject(new Error(`Flash failed (code ${code}): ${stderr || stdout}`));
    });
    esptool.on("error", (err) => { clearTimeout(timeout); reject(new Error(`Failed to run esptool: ${err.message}`)); });
  });
}

export async function getChipInfo(boardPath: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    esptool.on("close", (code) => {
      ...
      else reject(new Error(`Failed to get chip info: ${output}`));
    });
