...
export const MonitorSerialSchema = z.object({
  ...
  baud_rate: z.enum([...BAUD_RATES.map(String)] as any).transform(val => parseInt(val, 10)).default("115200" as any).describe("Serial baud rate"),
...
export async function monitorSerialTool(params: MonitorSerialInput) {
  let port;
  try {
    port = await openSerialConnection(params.board_path, params.baud_rate);
    const data = await readSerialData(port, params.timeout);
    return { content: [{ type: "text" as const, text: `📡 Serial Output (${params.baud_rate} baud)\n\n\`\`\`\n${data}\n\`\`\`` }] };
  } catch (error) {
    return { content: [{ type: "text" as const, text: `Error: ${error instanceof Error ? error.message : String(error)}` }] };
  } finally {
    if (port) await closeSerialConnection(port);
  }
}
