...
export async function flashFirmwareTool(params) {
    try {
        ...
        return { content: [{ type: "text", text: `✅ Firmware flashed successfully\n\n${result}` }] };
    }
    catch (error) {
        return {
            content: [{ type: "text", text: `❌ Flash failed: ${error instanceof Error ? error.message : String(error)}\n\nTroubleshooting:\n- Ensure board is connected\n- Hold BOOT button during flash\n- Check firmware path\n- Try lower baud rate (115200)` }]
        };
    }
}
